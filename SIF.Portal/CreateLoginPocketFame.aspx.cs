...
namespace SIF.Portal
{
    public partial class CreateLoginPocketFame : System.Web.UI.Page
    {
        ...
        protected void LoadRoles()
        {
            ...
            DataTable dtRoless = SqlHelper.Instance.GetTableByQuery(query);
        ...
        protected void Loadshifts()
        {
            ...
            DataTable dtshifts = SqlHelper.Instance.GetTableByQuery(query);
        ...
        public void GetData()
        {
            ...
            string ChkQry = "select * from Logindetails_Android where companyid=4 and EmpId='" + txtEmpid.Text + "' ";
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            int result = SqlHelper.Instance.ExecuteDMLQry(query);
            if (result > 0)
            {
                string ChkQry = "select * from Logindetails_Android where companyid=4 and EmpId='" + txtEmpid.Text + "' ";
                DataTable chkresult = config.PocketFameExecuteAdaptorAsyncWithQueryParams(ChkQry).Result;
                if (chkresult.Rows.Count > 0)
                {
                    string updatequery = "Update Logindetails_Android set UserName='" + txtusrname.Text.Trim() + "',Password='" + txtConfirmPassword.Text.Trim() + "',UpdatedDateTime=getdate(),Role='" + ddlrole.SelectedValue + "' where empid='" + Empid + "' and companyid=4";
                ...
                {
                    string insertquery = "insert into Logindetails_Android (Id,	EmpId,	UserName,	Password,	CompanyId,	CreatedDateTime	,UpdatedDateTime,	EmailId	,Role,	PushCode,	Rated	,ClientId) values (newid(),'" + txtEmpid.Text + "','" + txtusrname.Text.Trim() + "','" + txtConfirmPassword.Text.Trim() + "',4,Getdate(),'','','" + ddlrole.SelectedValue + "','',0,'')";
